...
void bcmsdh_oob_intr_set(bcmsdh_info_t *bcmsdh, bool enable)
{
	...
	if (bcmsdh_osinfo->oob_irq_enabled != enable) {
		...
		/* if (enable) */
		/* 	sw_gpio_eint_set_enable(bcmsdh_osinfo->oob_irq_num, 1); */
		/* else */
		/* 	sw_gpio_eint_set_enable(bcmsdh_osinfo->oob_irq_num, 0); */
#else
		if (enable)
			enable_irq(bcmsdh_osinfo->oob_irq_num);
		else
...
/* { */
/* 	bcmsdh_info_t *bcmsdh = (bcmsdh_info_t *)para; */
/* 	bcmsdh_os_info_t *bcmsdh_osinfo = bcmsdh->os_cxt; */
/* 	unsigned int penable = 0; */
/* 	int ret = -1; */

/* 	ret = sw_gpio_eint_get_enable(bcmsdh_osinfo->oob_irq_num, &penable); */
/* 	if (ret == 0 && penable == 1) */
/* 		wlan_oob_irq(0, para); */
/* 	return 0; */
/* } */
...
#elif defined(CUSTOMER_HW_ALLWINNER)
	err = devm_request_irq(bcmsdh_osinfo->dev, bcmsdh_osinfo->oob_irq_num, wlan_oob_irq,
		bcmsdh_osinfo->oob_irq_flags, "bcmsdh_sdmmc", bcmsdh);
 	/* irq_hand = sw_gpio_irq_request(bcmsdh_osinfo->oob_irq_num, */
 	/* 	bcmsdh_osinfo->oob_irq_flags, eint_handle, bcmsdh); */
	/* if(0 == irq_hand) { */
	/* 	err = -1; */
...
#ifdef CUSTOMER_HW_ALLWINNER
	//if(0 != irq_hand)
	  devm_free_irq(bcmsdh_osinfo->dev,bcmsdh_osinfo->oob_irq_num,NULL);
