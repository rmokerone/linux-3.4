...
static int usb_vbus_power_ctrl;
static int usb_eth_power_ctrl;
...
void ap6xxx_gpio_init(void)
{
  ...
  type = script_get_item(wifi_para, "wifi_power_switch", &val);
	...
	type = script_get_item(wifi_para, "usb_vbus_power_ctrl", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type)
		printk("get usb vbus power control gpio failed\n");
	else
		gpio_p = &val.gpio;
	usb_vbus_power_ctrl = gpio_p->gpio;
	sunxi_ap6xxx_gpio_req(gpio_p);
	printk("******************************** gpio vbus");

	type = script_get_item(wifi_para, "usb_eth_power_ctrl", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type)
		printk("get usb ethernet power control gpio failed\n");
	else
		gpio_p = &val.gpio;
	usb_eth_power_ctrl = gpio_p->gpio;
	sunxi_ap6xxx_gpio_req(gpio_p);
